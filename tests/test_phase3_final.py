"""
GTO-GameFlow v5.9.2 — Phase 3 Final: 全整合回测
- 真实赔率数据 (football-data.co.uk)
- 真实正交因子 (RealOddsEnhancer: 赔率变动/离散度/德比/欧战周)
- 修复串关生成 + 结算
- 动态 Kelly 分数
- 风控层
- 五大联赛统一阈值
...
from src.data.historical_odds_loader import load_odds_for_season, LEAGUE_CODES, DATA_DIR
...
from tests.test_phase2_optimization import (
    DynamicKellyEngine, RiskControlLayer,
)
from src.data.real_orthogonal_loader import create_enhanced_extra
from src.config.settings import GlobalConfig
version = GlobalConfig.version


def run_phase3_final_season(
    season: str,
    ...
    for lid in leagues:
        ...
        k_elo = {"premier_league": 24, "la_liga": 20, "bundesliga": 22, "serie_a": 20, "ligue_1": 20}.get(lid, 20)
        ...
        for round_idx, match in enumerate(odds_data.values()):
            try:
                ...
                match_date = datetime(2000, 1, 1)  # 默认日期
                ...
                # 使用真实正交因子增强器
                extra = create_enhanced_extra(
                    lid, season, match.home_team, match.away_team,
                ...
                if pipeline_result.placements:
                    ...
                    for p in placements:
                        ...
                        approved, adj_stake, _ = risk_control.check_bet(
                            p.stake, p.odds, shared_bankroll._get_base_bankroll(),
                            result["total_staked"], shared_bankroll.state.balance,
                        )
                        if not approved:
                    ...
                    # 每 20 场生成串关
                    if (processed + 1) % 20 == 0 or processed == len(odds_data) - 1:
    ...
    result["by_strategy"]["parlay"] = [result["parlay_bets"], result["parlay_wins"],
...
if __name__ == "__main__":
    print("=" * 78)
    print(f"  GTO-GameFlow {version} Phase 3 Final: 全整合 10赛季回测")
    print("=" * 78)
    print("  配置: 真实赔率 + 真实正交因子 + 修复串关 + 动态Kelly + 风控")
    ...
    print(f"  {'赛季':<10} {'投注':>6} {'单场':>6} {'串关':>6} {'胜率':>7} {'ROI':>8} {'利润':>10}")
    print(f"  {'-'*60}")

    for season in seasons:
        r, elos = run_phase3_final_season(season, 10000.0, elos)
        all_results.append(r)
        for k in totals:
            totals[k] += r.get(f"total_{k}", 0) if k != "bets" and k != "wins" else r.get(f"total_{k}", 0)
    ...
    for lid in ["premier_league", "la_liga", "bundesliga", "serie_a", "ligue_1"]:
        ...
        if lb > 0:
            ...
            print(f"  {lid:<16} {lb:>6} {lwr:>6.1%} {lroi:>+7.1%} {lr-ls:+10,.0f}")
    ...
    print(f"\n{'='*78}")
    print(f"  v{version} Phase 3 Final 完成")
    print(f"{'='*78}")